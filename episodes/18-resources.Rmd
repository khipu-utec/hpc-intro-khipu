...
impacto usando ``r config$sched$hist``. Puede usar este conocimiento para configurar el
...
``r config$sched$hist`` para obtener estadísticas sobre `parallel-job.sh`.
...
Pista: use ``r config$sched$flag$time``.

:::::::::::::::  solution

## Solución

La siguiente línea le indica a `r config$sched$name` que nuestro trabajo debería
