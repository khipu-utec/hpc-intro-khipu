...
El planificador usado en esta lección es `r config$sched$name`. Aunque
`r config$sched$name` no se usa en todos lados, ejecutar trabajos es bastante
...
- En la primera línea, agrega ``r config$remote$shebang``. El `#!`
...
comando ``r config$sched$status` `r config$sched$flag$user``.
...
ahora, cuando ``r config$sched$status`` muestra que el trabajo terminó,
...
(normalmente verás ``r config$local$shebang``). Planificadores como
`r config$sched$name` también tienen un comentario especial que se usa para
indicar opciones específicas del planificador. Aunque estos comentarios difieren
entre planificadores, el comentario especial de `r config$sched$name` es
``r config$sched$comment``. Cualquier cosa que siga al comentario
``r config$sched$comment`` se interpreta como una instrucción al planificador.

Ilustremos esto con un ejemplo. Por defecto, el nombre de un trabajo es el
nombre del script, pero la opción ``r config$sched$flag$name`` se puede usar
...
- `--nodes=<nnodes>` o `-N <nnodes>`: ¿En cuántas máquinas separadas necesita
  ...
  mayor que lo que puede ofrecer una sola máquina, `r config$sched$name`
...
¿Por qué el tiempo de ejecución de `r config$sched$name` y el tiempo de `sleep`
no son idénticos?
...
memoria de un nodo, `r config$sched$name` restringirá su trabajo a los recursos
...
hacer con el comando ``r config$sched$del``. Enviemos un trabajo y luego
...
``r config$sched$name`` también ofrece la posibilidad de iniciar una sesión
interactiva.
...
``r config$sched$interactive``.

``r config$sched$interactive`` ejecuta un solo comando en el clúster y luego
sale. Demostremos esto ejecutando el comando `hostname` con
``r config$sched$interactive``. (Podemos cancelar un trabajo de
``r config$sched$interactive`` con `Ctrl-c`.)
...
``r config$sched$interactive`` acepta todas las mismas opciones que
``r config$sched$submit$name``. Sin embargo, en lugar de especificarlas en un
...
mal en un trabajo anterior. Afortunadamente, `r config$sched$name` facilita
iniciar un trabajo interactivo con ``r config$sched$interactive``:
...
sesión con el comando `ssh`, por ejemplo, `ssh -X `r config$remote$user`@`r config$remote$login``.
...
``r config$sched$interactive`` con el comando
